...
''' 
if not os.path.exists("Pretreatment2"):
    os.mkdir("Pretreatment2")
    
path = r'F:\License_plate\tf_car_license_dataset_jb51\CCPD2019\ccpd_base'

for img_name in os.listdir(path):
    
    imgname = path + "/" + img_name
    
    img, img_Gas, img_B, img_G, img_R, img_gray, img_HSV = imgProcess(imgname)
    # 初步识别
    ...
    cv2.imwrite(r'\Pretreatment2' + img_name, lic)
'''
def run(imgpath):     

    img, img_Gas, img_B, img_G, img_R, img_gray, img_HSV = imgProcess(imgpath)
        # 初步识别
    img_bin  = preIdentification(img_gray, img_HSV, img_B, img_R)
    points = fixPosition(img, img_bin)
    vertices, rect = findVertices(points)
    ...
    cv2.namedWindow("Lic")
    cv2.imshow("Lic", lic)
    cv2.imwrite('Pretreatment.jpg', lic)
    # 暂停、关闭窗口
    cv2.waitKey(0)
