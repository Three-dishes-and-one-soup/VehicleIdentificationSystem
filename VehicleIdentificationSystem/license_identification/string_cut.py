...
img = cv2.imread("Pretreatment.jpg") # 读取图片 
...
# 3、保存黑白图片 
cv2.imwrite('thre_res.jpg', img_thre) 
...
def find_end(start_): 
  ...
  for m in range(start_+1, width-1): 
    if (black[m] if arg else white[m]) > (0.8 * black_max if arg else 0.8 * white_max): # 0.95这个参数请多调整，对应下面的0.05 
      end_ = m 
      break 
  return end_ 
