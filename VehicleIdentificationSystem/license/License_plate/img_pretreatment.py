...
def part_name():
    ...
    partname = partname.rsplit(".")
    partname = str(partname[0]+partname[1])
    return partname
...
def tiltCorrection(vertices, rect):
    # 畸变情况1
    if rect[2] == 0:
        new_right_point_x = vertices[3, 0]
        new_right_point_y = vertices[3, 1]
        new_left_point_x = vertices[2, 0]
        new_left_point_y = vertices[2, 1]
